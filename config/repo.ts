import {
  Apple,
  BookMarked,
  CalendarHeart,
  Clock,
  Gamepad2,
  Music4,
} from 'lucide-react'

export const REPO_LINKS = [
  {
    title: 'Eletrohits',
    description: 'A streaming music app',
    repo_url: 'https://github.com/vinihvc/eletrohits',
    website_url: 'https://eletrohits.vini.one',
    icon: Music4,
  },
  {
    title: 'Challenge Books',
    description: 'Google books, with search and filter',
    repo_url: 'https://github.com/vinihvc/challenge-books',
    website_url: 'https://books.vini.one',
    icon: BookMarked,
  },
  {
    title: 'Gradient Clock',
    description: 'Beautiful gradient clock',
    repo_url: 'https://github.com/vinihvc/gradient-clock',
    website_url: 'https://clock.vini.one',
    icon: Clock,
  },
  {
    title: 'Adventure Game',
    description: 'Idle game with a lot of adventures',
    repo_url: 'https://github.com/vinihvc/adventure',
    website_url: 'https://adventure.vini.one',
    icon: Gamepad2,
  },
  {
    title: 'Date Formatter',
    description: 'A date formatter that uses the Intl API',
    repo_url: 'https://github.com/vinihvc/date',
    website_url: 'https://date.vini.one',
    icon: CalendarHeart,
  },
  {
    title: 'Fit app',
    description: 'Nutritional table app',
    repo_url: 'https://github.com/vinihvc/fit',
    website_url: 'https://fit.vini.one',
    icon: Apple,
  },
]
